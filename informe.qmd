...
Si bien en la práctica, los intervalos de predicción pueden llegar a ser confusos para personas no técnicas sobre estadística es importante destacar su utilidad en la toma de decisiones, cualquier metodología que aborde sobre la mejora de estos, es de gran relevancia como la metodología expuesta en este articulo.

Con este articulo hemos revisado como implementar intervalos de predicción conformales para cualquier modelo de regresión, estos alogritmos requieren únicamente asumir que los datos tienen independencia entre sí, pero ninguna forma funcional sobre los regresores, matriz de diseño o distribución de probabilidad para los residuos. Esto último puede llegar a ser una gran ventaja para modelos en donde no es tan claro la construcción de intervalos de predicción que usualmente son aproximados con boostrap, como pueden ser los métodos de ensamble o en aquellos modelos donde se hacen supuestos fuertes sobre la distribución de los datos basados en resultados asintóticos.

El considerar este tipo de metodologías nos permite reducir la incertidumbre respecto a un intervalo de predicción clásico como pudimos ver en el @sec-teorema1. Es importante mencionar que se cuenta con una implementación por parte de los autores en la librería **conformalInferece** (ver @sec-conformalInferece) de R con modelos de regresión ampliamente utilizados en la práctica, como son modelos de regresión lineal, splines, glm, random forest, etc.

Cabe destacar que la inferencia comformal es aplicada frecuentemente en la actualidad a modelos de machine learning. Estas metdologías pueden complementar los métodos basados únicamente en boostrap para estimación de varianzas en para cualquier algortimo de regresión, demostrando que en base al estimador consistente, se puede obtener un intervalo de predicción cosistente en la estimación de la cobertura condicional 
...
# Paquete conformalInference {.appendix} {#sec-conformalInferece}


Existe un paquete disponible en CRAN, que ya tiene implementado todas estas herramientas.

En concreto, tenemos la función `conformal.pred` que nos permite realizar predicciones conformales para cualquier modelo de regresión, además de su versión menos costosa en términos computacionales `conformal.split`.


Antes de ir a un ejemplo práctico, veamos que argumentos recibe la función `conformal.pred`:
...
La documentación entera se encuentra en el siguiente link: \url{https://github.com/ryantibs/conformal/blob/master/conformalInference.pdf}


Para cada argumento *.fun se tienen implementado los siguientes modelos por defecto:

- **Modelo de regresión lineal** (Implementación propia de MCO usando descompocisión de Cholesky) `lm.funs()`
