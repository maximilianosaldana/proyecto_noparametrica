...
En la @fig-plot_simple se presenta tanto los intervalos con el método conformal simple como con el naive. Se puede notar que los primeros resultan un tanto más angostos que los segundos. La rugosidad se debe a que estamos trabajando en una grilla y no el recorrido continuo.
...
splitConfPredict <- function(Xin){
  ...
  resOut <- abs(subset(regData, split==2)$Y - predict(fitlm.spl, newdata=subset(regData, split==2)))
